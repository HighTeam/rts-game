...
fails the pack. `look_here` → `sfx/Cringemarine/look-here.wav` is force-tracked (silent PCM
placeholder; replace with real VO when available). Runtime loads the same path via
