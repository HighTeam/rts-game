...
M2 is on `main`: 2-player lockstep, graphical host/join, disconnect AI fallback, snapshot reconnect. M3 started with multi-peer transport (PR #38) plus `--lockstep-4-smoke` / `--lockstep-4-disconnect-smoke`. Graphical host/join is still 2-player. See [LOCKSTEP.md](docs/LOCKSTEP.md) for remaining silence-AI and N-way gaps.
