    ...
    const render::SimRenderSnapshot* render_snapshot)
{
    if (const auto* key_pressed = event.getIf<sf::Event::KeyPressed>()) {
        if (key_pressed->code == sf::Keyboard::Key::W && selection_.building != entt::null) {
            const bool is_town_center = render_snapshot != nullptr
                ? render::snapshot_has_town_center(*render_snapshot, selection_.building)
                : simulation.registry().any_of<sim::components::TownCenterTag>(selection_.building);
            if (is_town_center) {
                sim::player::PlayerCommand command{};
                if (render_snapshot != nullptr) {
                    command.execute_tick = render_snapshot->tick_count
    ...
    if (const auto* mouse_pressed = event.getIf<sf::Event::MouseButtonPressed>()) {
        if (mouse_pressed->button == sf::Mouse::Button::Left) {
            ...
            if (render_snapshot != nullptr) {
                ...
                const entt::entity town_center = render::pick_player_building_at_screen(
                    *render_snapshot,
                    renderer,
                    screen_position,
                    pick_radius_px,
                ...
                if (town_center != entt::null
                    && render::snapshot_has_town_center(*render_snapshot, town_center)) {
