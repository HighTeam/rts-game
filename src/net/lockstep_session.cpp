...
void LockstepSession::process_polled_messages_locked()
{
    ...
    while (transport_.consume_peer_lost_slot(lost_client_slot) && match_started_ && !host_lost_) {
        if (role_ == LockstepRole::Host) {
            if (awaiting_reconnect_handshake_) {
                awaiting_reconnect_handshake_ = false;
                client_resync_ready_ = false;
                ...
                if (session_player_count_ == 2U && !ai_fallback_) {
                    ...
                    note_player_slot_transport_down(lost_client_slot);
                }

                LockstepDebugLog::log_event(
                    "peer_lost_during_handshake",
                    "tick=" + std::to_string(simulation_.tick_count()) + " slot="
                    + std::to_string(lost_client_slot));
                continue;
            }
            ...
            LockstepDebugLog::log_event(
                "peer_lost",
                "tick=" + std::to_string(simulation_.tick_count()) + " slot="
                + std::to_string(lost_client_slot) + " grace_ms="
