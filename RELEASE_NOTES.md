...
Lobby clients must share the same `GAME_VERSION` (exact string match). CLI `--lockstep-join` does not enforce that gate.
...
- AoE Style HUD no longer draws the Default HUD minimap double-diamond stroke
...
- Training that finishes at 100% but cannot spawn because of **pop cap** refunds the cost and unlocks Town Center / Barracks / Mage Academy (blocked spawn tile with free pop still leaves the building locked — draft fix **#77**)
