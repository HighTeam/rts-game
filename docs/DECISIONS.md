...
| Singleplayer delay | `PLAYER_COMMAND_DELAY_TICKS = 1` (issue during tick *N* → execute at *N+1*) |
...
## Lockstep input delay (M2)

| Topic | Choice |
|-------|--------|
| Network delay | `LOCKSTEP_COMMAND_DELAY_TICKS = 2` in `src/net/net_constants.hpp` — earliest execute = `tick_count + 2` |
| Buffering | Local clicks stay in the `LockstepSession` outbox only. Peers send a `TickInputBatch` per execute tick (empty allowed), then both flush via `enqueue_network_command` before `Simulation::tick()` |
| Why not delay 1 | With delay 1, a click after the local batch was already sent applied on one peer early and desynced move orders (fixed in PR #33) |
| Hash exchange | After each tick, peers send `TickStateHash`; mismatch sets desync and stops advance |

Runbook: [LOCKSTEP.md](LOCKSTEP.md).
