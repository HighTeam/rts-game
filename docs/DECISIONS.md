...
8. **Host leave** — host `request_voluntary_resign` enqueues `HostEnded` so clients should treat the match as over (no reconnect to a dead host). On `main`, teardown often races the reliable send (draft **#101**); see [LOCKSTEP.md](LOCKSTEP.md).
