...
Step 0 runs **before** the movement-query cache. Steps 1–17 sit between
`begin_movement_query_cache` / `end_movement_query_cache` (path queries only; not hashed separately):
...
1. `run_building_process_system` — train / research / age-up / construction progress. Training that
   finishes at 100% but cannot spawn refunds only when `!player_can_spawn_units` (pop cap). If
   `try_complete_trained_unit_spawn` fails on a blocked tile with free pop, the building stays
   locked and cost is kept (draft fix PR **#77**). `issue_advance_age_order` only rejects an active
   process on the chosen Town Center — two TCs can age-up in parallel on `main` (draft fix PR **#83**)
2. `run_visibility_system` — fog explored / memory planes. Lobby / SP setup uses
   `FogOfWarMode` (`match_session.hpp`): **Enabled** (0) normal FoW, **Explored** (1) calls
   `reveal_all_explored` at match start then keeps visibility ticking, **Disabled** (2) turns
   fog off (`fog_of_war_enabled = false`). Wire field is `LobbySettings::fog_mode`.
...
| SFX | `SfxEventKind::LookHere` → `sfx/Cringemarine/look-here.wav` (silent PCM placeholder until real VO lands) |
...
HUD Diplomacy panel (`src/app/diplomacy.hpp`, input in `game_input.cpp`) submits ordinary
`PlayerCommand`s — same delay / lockstep outbox path as move/train.

| Action | Command | Wire payload packing |
|--------|---------|----------------------|
| Research trades | `ResearchTrades` (35) | Building process on Market; sets `MatchSession::player_trades[slot]` |
| Send resources | `SendTrade` (36) | `cell.x` = target slot, `cell.y` = wood; `goal_world_x/y` = food/gold; `target_entity` wire = mana |
| Ally / enemy + ally victory | `SetDiplomacy` (37) | `cell.x` = ally mask, `cell.y` = ally-victory flag |
| Chat | `NetMessageKind::Chat` (not a sim command) | Lockstep relays; Allies subtab filters by ally mask |

Constraints: Teams tab respects `block_team_changes` when set on the match session. Empty trade rows are ignored. Chat is UI/network only and does not affect `state_hash`.
