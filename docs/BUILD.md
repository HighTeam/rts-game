...
CLI `--lockstep-join` does **not** check `GAME_VERSION` (see [LOCKSTEP.md](LOCKSTEP.md)).
...
enqueues `HostEnded`). On `main`, peers often miss that message and hit the reconnect loop first
(draft **#101**); see [LOCKSTEP.md](LOCKSTEP.md).
...
| `--loose-assets` | Prefer loose `assets/` + `data/` next to the exe (or under `AOA_RUNTIME_ROOT`) instead of opening `assets.dat` first — Debug/dev packing iteration; see [assets/README.md](../assets/README.md) |
| `--lockstep-debug` | With `--lockstep-host` / `--lockstep-join`: write `logs/lockstep_pN_host.log` or `logs/lockstep_pN_client.log` next to the exe (`default_logs_directory()` in `runtime_paths.cpp`) |
| `--lockstep-auto-input` | With lockstep host/join: inject random local commands every 10 ticks after tick 5 (`lockstep_auto_input.cpp`) — LAN stress only, not CI |
| `--console-debug` | Windows GUI binary (`WIN32_EXECUTABLE`): attach/alloc a console so stdout/stderr are visible. Headless, harness, most smokes, `--print-hash`, and `--help` already request a console in `main.cpp` |

**Console pitfall:** `--lockstep-4-disconnect-smoke` and `--lockstep-peer-silence-smoke` are **not** in the auto-console list. On Windows, pass `--console-debug` (or redirect) if you need to read their stdout locally.

Local-only (not in `.github/workflows/build.yml`; ports in [LOCKSTEP.md](LOCKSTEP.md)):

| Mode | Use |
|------|-----|
| `--lockstep-2h2ai-smoke` | 4-slot lobby with 2 humans + 2 AI; human client leave → host AI takeover keeps advancing (`27205`) |
| `--lockstep-4-stress-smoke` | Four peers, empty batches, longer run (`LOCKSTEP_4_STRESS_TICKS` = 500) (`27206`) |
| `--sim-8ai-bench` | Single-process 8-player sim timing (idle / hash / AI) — not a network smoke |
| `--snapshot-double-spawn-smoke` | Snapshot round-trip then dual spawn-worker apply (entity-key wire path) |
| `--snapshot-reconnect-smoke` | Long tick + AI inject, then encode/apply snapshot round-trip |
| `--snapshot-heavy-smoke` | Busy command stream then encode/apply snapshot round-trip |

`main.cpp` dispatch (first matching flag wins): harness → net/lockstep/snapshot smokes →
`--lockstep-host` / `--lockstep-join` → headless → main menu / graphical.
...
Multi-process helpers: `scripts/run-lockstep-4-graphical.ps1`, `scripts/run-lockstep-4-stress.ps1`,
`scripts/run-lan-host.ps1`, `scripts/run-lan-join.ps1`.
...
**Pack constraint:** `aoa_pack_assets` (`tools/pack_assets.cpp`) packs (1) media paths regex-scraped
from `assets/visuals.json`, (2) cursor PNGs under `textures/CursorCrystal/`, and (3) every file
under `data/`. Paths listed in `visuals.json` that are missing on disk are **skipped with a
warning** so CI / clean clones still get a pack from the force-tracked subset (full art lives in
local `raw-assets/`). Silent PCM placeholders are force-tracked for map-ping
(`look_here` → `sfx/Cringemarine/look-here.wav` / `SFX_LOOK_HERE_RELATIVE_PATH`) and age-up
(`new_age` → `sfx/Cringemarine/new-age.wav` / `SFX_NEW_AGE_RELATIVE_PATH`) until real VO lands.

## Pattern Maker

Separate CMake target `aoa_pattern_maker` → `PatternMaker.exe` (optional NSIS component).

```powershell
cmake --build --preset x64-debug --target aoa_pattern_maker
.\build\x64-debug\Debug\PatternMaker.exe
```

Author map patterns under `patterns/` (JSON via `sim/map/map_pattern.*`). The game lobby map
picker and POST_BUILD copy use that folder next to the exe. Do not confuse with scenario JSON
under `data/scenarios/` (harness / hash regression).

Toolbar (verified in `src/pattern_maker/main.cpp`): New / Open / Save / Export, Undo / Redo,
place kinds (Start / Multi-start / Forest / Gold / Terrain / Mana lake / Berries / Middle point),
relative + road linkers, preview / validate, toggles for random gold/berries, fixed size, ring
mode, border/stray trees. Files are `.pattern` (and `.raw.pattern` import); keep authored
patterns under repo `patterns/`.
...
Output: `build\x64-release\Release\aoa-setup.exe`. Payload (from `scripts/package-setup.ps1` + `packaging/aoa-setup.nsi`):

| Item | Notes |
|------|-------|
| `AgeofAffinities.exe` + SFML/runtime DLLs | Required |
| `assets.dat` | Required — setup fails staging if missing |
| `scenarios\` / `patterns\` | Copied from the Release output folder |
| `EULA.txt` | Staged from `legal/EULA.md` (license page + install dir) |
| `RELEASE_NOTES.md` | Finish-page README + Start Menu shortcut |
| Pattern Maker | Optional NSIS component when `PatternMaker.exe` is present in the Release folder |
| VC++ x64 redistributable | Optional section when `vc_redist.x64.exe` is found under the local VS 2022 Redist tree; otherwise the script warns and skips it |

`aoa_pack_assets.exe` is not shipped. Display version comes from `GAME_VERSION` in `src/core/constants.hpp`; product version from the CMake `project(... VERSION ...)`.
