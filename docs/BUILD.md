...
under `data/`. A path listed in `visuals.json` that is missing on disk aborts POST_BUILD and fails
CI before any smoke runs. Map-ping SFX path `look_here` → `sfx/Cringemarine/look-here.wav` is
tracked (silent PCM placeholder until real VO lands; same path as `SFX_LOOK_HERE_RELATIVE_PATH`).
