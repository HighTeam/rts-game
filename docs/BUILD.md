...
**Pack pitfall:** `aoa_pack_assets` (`tools/pack_assets.cpp`) packs (1) media paths regex-scraped from
`assets/visuals.json`, (2) cursor PNGs under `textures/CursorCrystal/`, and (3) every file under
`data/`. A path listed in `visuals.json` that is missing on disk aborts POST_BUILD and fails CI
before any smoke runs. Current blocker: `look_here` → `sfx/Cringemarine/look-here.wav` (also
`SFX_LOOK_HERE_RELATIVE_PATH` in `constants.hpp` for runtime map-ping SFX).
