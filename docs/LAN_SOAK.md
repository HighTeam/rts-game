...
With `--lockstep-debug`, logs are written under **`logs/` next to the executable**
(`default_logs_directory()` → e.g. `D:\aoa-lan\logs\lockstep_p1_host.log`,
`lockstep_p2_client.log`). Slot in the filename is `player_slot + 1`.
