...
- [ ] Optional: freeze P2 without killing it → after ~3s host silence (`LOCKSTEP_PEER_SILENCE_MS`) may trip AI while the window is still open. On `main`, expect P2 to hang waiting for host batches until you kill/restart it (open fix #67 should disconnect the peer so P2 can reconnect)
...
| P2 frozen after host silence AI, session still “connected” | Host entered AI without dropping the peer; see [LOCKSTEP.md](LOCKSTEP.md) Silence-based AI (open fix #67) |
