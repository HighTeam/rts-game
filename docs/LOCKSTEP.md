...
1. Starts `LOCKSTEP_RECONNECT_GRACE_MS` (30s) and sets `opponent_reconnect_pending_`
2. Calls `enter_ai_fallback()` on the same tick transport drops (AI is immediate; grace does **not** delay takeover)
...
The 30s grace does **not** close the reconnect window. Mid-match reconnect still works after grace expires. On this immediate-AI path, `check_opponent_reconnect_timeout()` returns early because `ai_fallback_` is already set, so grace expiry never clears `opponent_reconnect_pending_` — the host title can stay on “AI playing (waiting for reconnect)” until a real reconnect. Grace is restarted after resync and mainly suppresses silence-based re-AI (below).
...
If the host drops, the client stops simulating, retries reconnect every `LOCKSTEP_RECONNECT_INTERVAL_MS` (500) up to `LOCKSTEP_RECONNECT_MAX_ATTEMPTS` (20) ≈ **10s**, then sets `host_gone_` and exits. The client does **not** AI-control the host. Host migration remains M3.

### Silence-based AI (still “connected”)

Besides hard disconnect, the host can enter the same AI path while ENet still reports a peer: `is_opponent_unresponsive()` is true when the host is waiting on a remote `TickInputBatch` for longer than `LOCKSTEP_PEER_SILENCE_MS` (3000) after hash-verify warmup, and reconnect grace is not active. `try_advance_live_tick()` then calls `note_opponent_transport_down()` → immediate AI. Treat a sudden “Player 2 left — AI playing” with the client still open as a stalled batch / silence trip, not only a socket drop.

**2-player client freeze on silence (main today).** `note_opponent_transport_down()` sets `opponent_reconnect_pending_ = true` before `enter_ai_fallback()`. The host’s disconnect guard in `enter_ai_fallback()` still requires `!opponent_reconnect_pending_`, so a silence trip never calls `transport_.disconnect_peer()` while the ENet session stays up. The host then solo-advances in AI mode and stops sending batches/hashes. The silent client never sees transport down, never enters `host_lost`/reconnect, and blocks forever in `try_advance_live_tick` waiting for host batches. Open fix PR #67 drops the peer on 2-player AI takeover when still connected and adds `--lockstep-peer-silence-smoke` (port `27203`); none of that is on `main` yet.
...
Client reconnect pacing when the **host** drops: every `LOCKSTEP_RECONNECT_INTERVAL_MS` (500), up to `LOCKSTEP_RECONNECT_MAX_ATTEMPTS` (20) ≈ 10s total, then “host left the game”. That budget is independent of the host’s 30s reconnect-grace constant (which does not reject late client returns after an AI takeover).
...
| Client gives up rejoining a lost host after ~10s | Client budget is `LOCKSTEP_RECONNECT_MAX_ATTEMPTS` × `LOCKSTEP_RECONNECT_INTERVAL_MS` (20×500ms), not the host’s 30s grace |
| Host still accepts reconnect after 30s of AI | Expected. Grace does not reject reconnect; see AI fallback |
| Title stays “waiting for reconnect” long after 30s | Immediate-AI path never clears `opponent_reconnect_pending_` on grace expiry |
| Opponent went AI while the client window is still open | Host silence trip (`LOCKSTEP_PEER_SILENCE_MS` = 3000) while waiting on a remote batch; see Silence-based AI |
| Client freezes after host shows AI, but the ENet session never dropped | Silence AI on host without `disconnect_peer()` (`opponent_reconnect_pending_` blocks the drop); client waits forever on host batches; open fix #67 |
