...
Verified against `main` @ `alpha_v0.2` (`ddffd27`): M2 reconnect stack (#40–#48, #59, #64, #67)
plus M3 multi-peer CLI (`--players` / `--player-slot`) and `--lockstep-4-reconnect-smoke`.
...
Magic `0x414F4153` (`AOAS`), `SNAPSHOT_VERSION = 25` (`sim_snapshot.cpp`).
...
`apply_sim_snapshot` resets via scenario load, then overlays map/fog/entities.
It does **not** rebuild the world by replaying `input_log`; the log is restored into
`CommandQueue` only. Entity ids are not stable across restore — use `EntitySnapshotKey`.

Alpha_v0.2 reconnect fix: snapshot restore must not inflate `player_stats` via `spawn_*`
(that caused immediate post-reconnect desync).
...
| Mid-barrier multi-peer AI takeover | `handle_host_player_slot_disconnected` / resync timeout call `reset_tick_sync_state()` after per-slot AI; can wipe live ready bits for other peers (draft fix PR #72) |
| CI smoke exit codes | PowerShell Headless smoke step does not fail the job on native non-zero exits |
| Voluntary leave / resign | Slot not reclaimable; must not trigger false AI takeover |
...
- [RELEASE_NOTES.md](../RELEASE_NOTES.md) — alpha_v0.2 reconnect hash / HUD notes
