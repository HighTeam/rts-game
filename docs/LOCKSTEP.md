...
Lobby UI and lockstep CLI joins both require an **exact** `GAME_VERSION` string match
(`src/core/constants.hpp`, currently `alpha_v0.2.1`).

| Path | Check |
|------|-------|
| Menu lobby | Host rejects `LobbyJoin` when `message.version != GAME_VERSION` → `LOBBY_VERSION_MISMATCH_MESSAGE` ("Version mismatch") |
| Lockstep join | Host accepts join only when `join->version == GAME_VERSION` (`lockstep_session.cpp`) |

Owner-gated bumps only — a version change intentionally splits playtest pools.
...
| `--lockstep-auto-input` | Injects random Move/Gather/Attack/Train/Build-style local commands on ticks `5,15,25,…` |
