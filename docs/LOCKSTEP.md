...
| Missing pack file breaks CI | `visuals.json` `look_here` → `sfx/Cringemarine/look-here.wav` is missing under `assets/`; `aoa_pack_assets` fails POST_BUILD (docs PRs inherit this until the asset lands; same path as `SFX_LOOK_HERE_RELATIVE_PATH`) |
