...
Open caveats (not fixed on `main`): draft PRs **#72**, **#75**, **#77**, **#79**, and **#81**
— see Remaining gaps.
...
player's batches as soon as the resign lands.

**Auth / desync caveats on `main` (draft fix PRs #79 / #81 — not merged):**
...
| Late multi-peer `ResyncReady` | After resync timeout, `handle_resync_ready` late-ack path resumes the slot on the host but does **not** `broadcast_slot_ai_resume` / catch-up snapshot to bystanders — host treats the slot as human while peers keep AI (draft fix PR **#75** — not merged) |
