...
`HostEnded`, snapshot v26 (map pings), second-reconnect handshake polish, and debug CLI
(`--lockstep-debug` / `--lockstep-auto-input` / `--console-debug`).
Open caveats (not fixed on `main`): draft PRs **#72**, **#75**, **#77**, **#79**, **#81**, **#83**,
**#85**, **#87**, **#89**, **#93**, **#95**, **#101**, and **#104** — see Remaining gaps.
Also documented without dedicated fix PRs yet: predictable menu claim tokens, and bystander
barrier wipe that survives #95's handshake gate.
...
Menu lobby joins require an **exact** `GAME_VERSION` string match
(`src/core/constants.hpp`, currently `alpha_v0.2.1`). CLI `--lockstep-host` /
`--lockstep-join` do **not** exchange or check `GAME_VERSION` on the wire
(`ReconnectRequest` carries only `player_slot` + `claim_token`).

| Path | Check |
|------|-------|
| Menu lobby | Host rejects `LobbyJoin` when `message.version != GAME_VERSION` → `LOBBY_VERSION_MISMATCH_MESSAGE` ("Version mismatch") |
| Mid-match menu rejoin | Host accepts `LobbyJoin` only when `join->version == GAME_VERSION` (`lockstep_session.cpp`) |
| CLI `--lockstep-join` | No version field / gate — mismatched builds can still connect if the wire stays compatible |

Owner-gated bumps only — a lobby version change intentionally splits playtest pools.
...
Repo conveniences (Release exe path hardcoded): `scripts/run-lockstep-4-graphical.ps1`
(4 windows on **27000**, debug + auto-input) and `scripts/run-lockstep-4-stress.ps1`
(4 headless peers on **27010**, 500 ticks). Details: [M3_SCALE_TESTING.md](M3_SCALE_TESTING.md) §2b.
Two-PC LAN helpers: `scripts/run-lan-host.ps1` / `scripts/run-lan-join.ps1` — [LAN_SOAK.md](LAN_SOAK.md) §5.
...
Local-only (not in CI; same binary):

```powershell
& $exe --lockstep-2h2ai-smoke          # 27205 — 2 human + 2 AI; client leave keeps host advancing
& $exe --lockstep-4-stress-smoke       # 27206 — 4 peers × 500 ticks
& $exe --sim-8ai-bench                 # no ENet — 8-slot sim timing
& $exe --snapshot-double-spawn-smoke
& $exe --snapshot-reconnect-smoke
& $exe --snapshot-heavy-smoke
```

`main.cpp` dispatch: harness → net/lockstep/snapshot smokes → lockstep host/join → headless → graphical.
First matching flag wins. Full flag table: [BUILD.md](BUILD.md).

### Debug flags (host / join)

| Flag | Effect |
|------|--------|
| `--lockstep-debug` | Opens `exe/logs/lockstep_pN_host.log` or `…_client.log` (`LockstepDebugLog::enable`) |
| `--lockstep-auto-input` | Injects random Move / Gather / SpawnWorker / SpawnMilitia local commands on ticks `5,15,25,…` (`lockstep_auto_input.cpp`; no Attack/Build) |
| `--console-debug` | Windows: force a console on the GUI-subsystem binary (needed for some smokes — see [BUILD.md](BUILD.md)) |
...
**Reconnect bind-before-auth on `main` (draft fix PR #104 — not merged):**
`handle_reconnect_request` calls `bind_player_to_enet_slot(player_slot, sender_enet_slot)` and
sets `pending_reconnect_enet_slot_` **before** `claim_token_accepted`. On mid-match reject
(`reason=claim_token`) the early return leaves the binding in place, so a forged
`ReconnectRequest` for another player's slot can permanently remap host routing (snapshots
and per-slot traffic go to the wrong ENet peer). CLI/smoke tokens of `0` accept any claim
(`claim_token_accepted`), so the auth miss mainly bites when a non-zero token was issued.
Draft **#104** validates claim token (and rejects cross-slot reclaim when the sender is
already mapped to a different player) **before** any bind.

**Predictable menu claim tokens (no fix PR yet):** menu matches build
`reconnect_tokens[]` with `reconnect_token_for_slot(slot, player_name)` in
`app_shell.cpp` — a deterministic FNV-style mix of compact slot + lobby name (`| 1` so
never zero). Every peer recomputes the full roster locally from the shared lobby view, so
a connected player who knows names can forge a valid `claim_token` for a **disconnected**
slot. Tightening this needs host-issued private tokens (e.g. via `JoinAccepted`), not just
#104's bind-order fix.
...
peers spam `ResyncReady`. Draft **#95** gates that handshake arm on
`actively_reconnecting || awaiting_reconnect_handshake_` only (still covers host snapshot
retries before `ResyncReady`).

**Bystander barrier wipe remains even after #95:** on `main`,
`reset_tick_sync_state()` + `transport_.discard_pending_received()` run **unconditionally**
after a successful snapshot apply (before the handshake/`state_changed` branch). Live
bystanders receiving a host catch-up therefore still clear `remote_ready_slots_by_tick_` /
pipelined barrier state. #95 stops the spurious `ResyncReady` path; it does **not** move
those resets under `actively_reconnecting`. Extend #95 (or a follow-up) so bystander
catch-up skips the barrier wipe.
...
| **Leave / Exit to menu** | `request_voluntary_resign` | Same resign marking; host also enqueues `HostEnded` then tears down transport |
| **HostEnded** (when received) | clients set `host_gone_` | Match ends for everyone — no reconnect to a dead host |

**HostEnded delivery gap on `main` (draft fix PR #101 — not merged):** `send_host_ended` +
`flush_outbound()` enqueue a reliable packet, but `disconnect_transport` /
`request_voluntary_resign` then call `disconnect()` / `disconnect_peer()` which use
`enet_peer_disconnect_now` and (for full teardown) destroy the host immediately. Clients
often never receive `HostEnded` and instead sit in the ~30s host-lost reconnect loop before
giving up. Intended UX is the "Host has ended the game" overlay as soon as leave happens.
...
| Bystander catch-up snapshot → spurious handshake | `handle_reconnect_snapshot` ORs `state_changed` into the handshake condition. Live peers receiving a host catch-up after another slot reconnects always see tick/hash advance and send `ResyncReady` (draft fix PR **#95** — not merged) |
| Bystander catch-up still wipes tick barrier | Even with #95, `reset_tick_sync_state()` + `discard_pending_received()` run unconditionally after snapshot apply — live bystanders lose `remote_ready_slots_by_tick_` during another slot's reconnect (extend **#95** / follow-up — no dedicated PR yet) |
| Predictable reconnect claim tokens | Menu `reconnect_token_for_slot(slot, name)` is deterministic and shared via the lobby roster; any peer can forge a valid reclaim for a disconnected slot (needs host-private tokens — no fix PR yet; orthogonal to **#104**) |
| CI smoke exit codes | PowerShell Headless smoke step does not fail the job on native non-zero exits |
| Map-ping / age-up SFX are silent placeholders | `sfx/Cringemarine/look-here.wav` and `sfx/Cringemarine/new-age.wav` are force-tracked so pack/CI succeed; both are silent PCM until real VO replaces them (`SFX_LOOK_HERE_RELATIVE_PATH`, `SFX_NEW_AGE_RELATIVE_PATH`). Other `visuals.json` media may be skipped if absent on disk (see [assets/README.md](../assets/README.md)) |
| `HostEnded` rarely reaches clients | Host leave flushes then `disconnect_now` / destroy before peers can receive the reliable message → reconnect loop instead of immediate host-ended overlay (draft fix PR **#101** — not merged) |
| Reconnect bind before claim-token auth | `handle_reconnect_request` binds `player_slot` → sender ENet slot and sets `pending_reconnect_enet_slot_` before `claim_token_accepted`; a rejected forged reclaim still remaps host routing (draft fix PR **#104** — not merged) |
