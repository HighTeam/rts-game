...
| Packed `assets.dat` + NSIS installer script | Shipped baseline (map-ping / age-up SFX are silent placeholders) |
...
- [ ] **Map-ping / age-up SFX VO** — replace silent `sfx/Cringemarine/look-here.wav` and `sfx/Cringemarine/new-age.wav` placeholders with real VO (`SFX_LOOK_HERE_RELATIVE_PATH`, `SFX_NEW_AGE_RELATIVE_PATH`)
- [ ] **Force-track remaining pack media** — HighTeam / Evergrow / biome textures referenced by `visuals.json` but not yet in git (pack currently skips missing paths)
...
- [ ] New ground tiles with walk/path rules beyond today’s lobby biome presets (Mixed / Grass / Snow / Sand already tint generation; Map Redesign still needs richer tile vocabulary + rules)
...
1. **Polish & Debt** — enough that the Earth build is trustworthy (incl. real map-ping VO for `look-here.wav`)  
